...
export class ChatRooms extends Component {
  ...
  handleNotification = (
    ...
    let lastTotal = 0;
    // 이미 notification state안에 알림 정보가 들어있는 채팅방과 그렇지 않은 채팅방을 나눠주기
    let index = notifications.findIndex(
      (notification) => notification.id === chatRoomId
    );
    // notifications state 안에 해당 채팅방에 알림 정보가 없을 때
    if (index === -1) {
      notifications.push({
        id: chatRoomId,
      ...
      if (chatRoomId !== currentChatRoomId) {
        ...
        // count (알람으로 보여줄 숫자)를 구하기
        // 현재 총 메시지 개수 - 이전에 확인한 총 메시지 개수 > 0
        // 현재 총 메시지 개수가 10개이고 확인한 메시지가 8개 였다면 2개를 알림으로 버여줘야 함
